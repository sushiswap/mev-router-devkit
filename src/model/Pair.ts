...
export class Pair {
  ...
  constructor(
    ...
    this.oracle = new Oracle(
      this.inverted ? second.decimals : first.decimals,
      this.inverted ? first.decimals : second.decimals,
      state.price,
    );
  }

  getSwapInput(output: CurrencyValue) {
  ...
  getSwapOutput(input: CurrencyValue) {
  ...
  getEstimatedSwapFee(input: BigNumber) {
  ...
  getSecondReserve() {
    return new CurrencyValue(
      this.second,
      this.inverted ? this.state.reserve0 : this.state.reserve1,
    );
  }

  getDepositReservesFraction(firstTokenPrice?: TokenPrice, secondTokenPrice?: TokenPrice) {
    const firstReserve = this.getFirstReserve();
    const secondReserve = this.getSecondReserve();
    if (firstTokenPrice && secondTokenPrice) {
      const firstReserveValue = normalizeTo18Decimals(
        firstReserve.value,
        firstReserve.currency.decimals,
      )
        .mul(firstTokenPrice.price)
        .div(BN_10_18);
      const secondReserveValue = normalizeTo18Decimals(
        secondReserve.value,
        secondReserve.currency.decimals,
      )
  ...
  getFirstLiquidityIn(secondLiquidityIn: CurrencyValue) {
    ...
    return new CurrencyValue(
      this.first,
      firstReserve.mul(secondLiquidityIn.value).div(secondReserve),
    );
  }

  getSecondLiquidityIn(firstLiquidityIn: CurrencyValue) {
    ...
    return new CurrencyValue(
      this.second,
      secondReserve.mul(firstLiquidityIn.value).div(firstReserve),
    );
  }

  getLiquidity(firstValue: CurrencyValue, secondValue: CurrencyValue, proportional: boolean) {
    if (this.inverted) {
      const { swapDeposit0Amount, swapDeposit1Amount, liquidity } = this.getRawLiquidity(
        secondValue,
        firstValue,
        proportional,
      );
  ...
  getBurnOutput(liquidity: BigNumber) {
    ...
    const first = this.getFirstReserve().map((value) =>
      value.mul(effectiveLiquidity).div(this.state.totalSupply),
    );
    const second = this.getSecondReserve().map((value) =>
      value.mul(effectiveLiquidity).div(this.state.totalSupply),
    );
    return { first, second };
  }
  ...
  private getSwapAmount0In(amount1Out: BigNumber) {
  ...
  private getSwapAmount1In(amount0Out: BigNumber) {
    const { reserve0, reserve1, swapFee } = this.state;
  ...
  private getSwapAmount0Out(amount1In: BigNumber, updatedReserves?: [BigNumber, BigNumber]) {
  ...
  private getSwapAmount1Out(amount0In: BigNumber, updatedReserves?: [BigNumber, BigNumber]) {
    const { reserve0, reserve1 } = updatedReserves
      ? {
          reserve0: updatedReserves[0],
          reserve1: updatedReserves[1],
  ...
  private getRawLiquidity(
    ...
    // reserves after _initialDeposit in executeDeposit
    ...
    const { extraLiquidity, swapDeposit0Amount, swapDeposit1Amount } = this.getExtraLiquidity(
      amount0Left,
      amount1Left,
      updatedReserve0,
      updatedReserve1,
    ...
    return {
      ...
      swapDeposit0Amount: CurrencyValue.fromString(
        currency0.currency,
        swapDeposit0Amount.lt(0)
          ? '0'
          : formatUnits(swapDeposit0Amount, currency0.currency.decimals),
      ),
      swapDeposit1Amount: CurrencyValue.fromString(
        currency1.currency,
        swapDeposit1Amount.lt(0)
          ? '0'
  ...
  private getExtraLiquidity(
    ...
    const extraRatio =
      !extraAmount0.isZero() && !extraAmount1.isZero()
  ...
  private initialDeposit(ratio: BigNumber) {
    const { reserve0, reserve1 } = this.state;
    return [ratio.mul(reserve0).div(PRECISION), ratio.mul(reserve1).div(PRECISION)];
  }

  private swapDeposit0(amount0: BigNumber, updatedReserve0: BigNumber, updatedReserve1: BigNumber) {
  ...
  private swapDeposit1(amount1: BigNumber, updatedReserve0: BigNumber, updatedReserve1: BigNumber) {
